...
public class ModConfigScreen extends Screen {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (mouseY >= listTop && mouseY <= listBottom && maxScroll > 0) {
            this.scrollAmount = Mth.clamp(this.scrollAmount - (int) (verticalAmount * 14), 0, maxScroll);
            updateLayout();
            return true;
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        for (ConfigRow row : rows) {
            if (row.visible) {
                row.renderLabel(context, this.font);
            }
        }
    ...
    private void renderListScrollbar(GuiGraphicsExtractor context) {
        int viewportHeight = listBottom - listTop;
        int contentHeight = rows.size() * ROW_HEIGHT;
    ...
    private static String prettyName(String raw) {
        // Split on camelCase boundaries instead of underscores
    ...
    private static final class ConfigRow {
        ...
        private void renderLabel(GuiGraphicsExtractor context, Font font) {
            context.text(font, prettyName(field.getName()), x, y + 6, 0xFFFFFFFF);
        }
